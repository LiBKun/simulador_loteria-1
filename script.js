...
function addToList(num, pos) {
  if(num.length == 2){
    if(numEsco.includes(num)){
      alert2("Número escolhido anteriormente. Digite outro número.")
    } else if (parseInt(num) > 60){
      alert2("O número digitado não pode ser maior que 60.")
    } else {
      numEsco[pos-1] = num;
    }
  }
...
function VerificarAcertos() {
  ...
  let cont = 0;
  if(numEsco.length != 6){
    alert2("A quantidade de números escolhidos é menor que 6.\n Digite 6 números de 01 a 60 com duas casas decimais.");
  } else {
    for (var i = 0; i < 6; i++) {
